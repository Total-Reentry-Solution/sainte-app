...
class WebGoalsPage extends HookWidget {
  ...
  Widget build(BuildContext context) {
    useEffect(() {
      ...
      context.read<GoalCubit>().fetchHistory();
    }, []);
...
class GoalsHistoryTable extends StatelessWidget {
  const GoalsHistoryTable({super.key, this.userId});
  final String? userId;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<GoalCubit, GoalCubitState>(
      builder: (context, state) {
        ...
        if (state.state is GoalSuccess) {
          List<GoalDto> history = state.history;
        ...
        Expanded(
          child: ClipRRect(
            ...
            child: LinearProgressIndicator(
              value: progress / 100,
              backgroundColor: AppColors.hintColor,
              color: AppColors.green, // Use green for completed goals
            ),
          ),
        ),
        const SizedBox(width: 10),
