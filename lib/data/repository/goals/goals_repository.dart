...
class GoalRepository {
  Future<List<GoalDto>> fetchActiveGoals({String? userId}) async {
  ...
  Future<List<GoalDto>> fetchGoalHistory({String? userId}) async {
    final id = userId ?? SupabaseConfig.currentUser?.id;
    if (id == null) throw BaseExceptions('User not found');
    final response = await SupabaseConfig.client
        .from('goals')
  ...
  Future<ProgressStats> fetchGoalStats({String? userId}) async {
    final id = userId ?? SupabaseConfig.currentUser?.id;
    if (id == null) throw BaseExceptions('User not found');
